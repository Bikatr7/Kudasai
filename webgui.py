                ...
                with gr.Tab("Kijiku Settings") as self.kijiku_settings_tab:
                    with gr.Row():
                        ...
                        with gr.Column():
                            ...
                            self.presence_penalty_input_field = gr.Textbox(label='Presence Penalty', info="Number between -2.0 and 2.0. Positive values penalize new tokens based on whether they appear in the text so far, increasing the model's likelihood to talk about new topics.", lines=1, max_lines=1, show_label=True, interactive=True)
                            self.frequency_penalty_input_field = gr.Textbox(label='Frequency Penalty', info="Number between -2.0 and 2.0. Positive values penalize new tokens based on their existing frequency in the text so far, decreasing the model's likelihood to repeat the same line verbatim.", lines=1, max_lines=1, show_label=True, interactive=True)
                            ...
                            self.message_mode_input_field = gr.Textbox(label='Message Mode', info="1 or 2. 1 means the system message will actually be treated as a system message. 2 means it'll be treated as a user message. 1 is recommend for gpt-4 otherwise either works.", lines=1, max_lines=1, show_label=True, interactive=True)
                            ...
                            self.sentence_fragmenter_mode_input_field = gr.Textbox(label='Sentence Fragmenter Mode', info="1 or 2 or 3 (1 - via regex and other nonsense, 2 - NLP via spacy, 3 - None (Takes formatting and text directly from ai return)) the api can sometimes return a result on a single line, so this determines the way Kijiku fragments the sentences if at all. Use 3 for gpt-4.", lines=1, max_lines=1, show_label=True, interactive=True)
                            self.je_check_mode_input_field = gr.Textbox(label='JE Check Mode', info="1 or 2, 1 will print out the jap then the english below separated by ---, 2 will attempt to pair the english and jap sentences, placing the jap above the eng. If it cannot, it will do 1. Use 2 for gpt-4.", lines=1, max_lines=1, show_label=True, interactive=True)
                            self.num_malformed_batch_retries_input_field = gr.Textbox(label='Number of Malformed Batch Retries', info="How many times Kudasai will attempt to mend a malformed batch, only for gpt4. Defaults to 1, careful with increasing as cost increases at (cost * length * n) at worst case.", lines=1, max_lines=1, show_label=True, interactive=True)
                            ...
                            self.num_concurrent_batches_input_field = gr.Textbox(label='Number of Concurrent Batches', info="How many translations batches Kudasai will send to OpenAI at a time.", lines=1, max_lines=1, show_label=True, interactive=True)
